...
<html>
    ...
    while($row=mysqli_fetch_assoc($result)){
        ...
        if($row['status']==0){
            $status="Not Logged in <span style='background-color:#d80000f0; border-radius:50%; height:10px; width:10px; display:inline-block;'></span>";
        }else if($row['status']==1){
            $status="Logged in <span style='background-color:#04ff04bd; border-radius:50%; height:10px; width:10px; display:inline-block;'></span>";
        }
